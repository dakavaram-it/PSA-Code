...
// ---------------------------------------------------------------------------
const USER_TYPES = [
  { id: 9, type: "KEY", short: "KEY" }, { id: 3, type: "MINISTER", short: "MINISTER" },
  { id: 6, type: "MP", short: "MP" }, { id: 7, type: "MLA", short: "MLA" },
  { id: 8, type: "CONSTITUENCY", short: "ACI" }, { id: 14, type: "PARLIAMENT PARTY PRESIDENT", short: "PPP" },
  { id: 15, type: "ZONAL COORDINATOR", short: "ZONAL" }, { id: 16, type: "PROGRAM COMMITTEE", short: "PROG CMTE" },
  { id: 11, type: "ECM_TEAM(RAJASHEKHAR)", short: "ECM" }, { id: 12, type: "OBSERVER", short: "OBSERVER" },
  { id: 5, type: "LN TEAM", short: "LN TEAM" }, { id: 4, type: "TEST USER", short: "TEST USER" },
  { id: 18, type: "OTHERS", short: "OTHERS" }, { id: 19, type: "MYTDP APP TEAM", short: "MYTDP APP" },
  { id: 1, type: "COUNTRY", short: "COUNTRY" }, { id: 2, type: "STATE", short: "STATE" },
  { id: 17, type: "AC OBSERVER", short: "AC OBSERVER" },
];
const USER_LEVELS = [
  { id: 1, name: "COUNTRY" }, { id: 2, name: "STATE" }, { id: 3, name: "DISTRICT" },
  { id: 4, name: "PARLIAMENT" }, { id: 5, name: "ASSEMBLY" }, { id: 6, name: "MANDAL" },
  { id: 7, name: "MUNICIPALITY" }, { id: 8, name: "VILLAGE" }, { id: 9, name: "WARD" },
];
const USED_LEVEL_IDS = [5, 4, 2];
const COMPONENTS = [
  ...
  { id: 120, name: "programsDayWise", actual: "Programs Day-wise", display: null },
  { id: 133, name: "suparipalana", actual: "Su-Governance First Step", display: "సుపరిపాలనలో తొలి అడుగు" },
  { id: 103, name: "D2DCampaignHouseVisitNew", actual: null, display: null },
...
// --- Mock tdp_cadre lookup (real: 22.5M rows, indexed on membership_id). -----
// Point lookup by MID only — never scanned. A handful of seed rows so the
// MID-first create flow can resolve some IDs and miss others.
const CADRE_BY_MID = {
  "20481": { tdp_cadre_id: 500481, first_name: "PRIYA", last_name: "REDDY", mobile_no: "9876543210", gender: "F", constituency: "Tirupati", payment_status: "PAID" },
  "20482": { tdp_cadre_id: 500482, first_name: "SURESH", last_name: "NAIDU", mobile_no: "9812345678", gender: "M", constituency: "Guntur East", payment_status: "PAID" },
  "20483": { tdp_cadre_id: 500483, first_name: "LAKSHMI", last_name: "RAO", mobile_no: "9898989898", gender: "F", constituency: "Kurnool", payment_status: "NOT PAID" },
  "30017": { tdp_cadre_id: 530017, first_name: "VENKAT", last_name: "CHOWDARY", mobile_no: "9765432109", gender: "M", constituency: "Nellore City", payment_status: "PAID" },
  "30018": { tdp_cadre_id: 530018, first_name: "SANDHYA", last_name: "VARMA", mobile_no: "9700011223", gender: "F", constituency: "Eluru", payment_status: "NR" },
};
const lookupCadre = (mid) => CADRE_BY_MID[String(mid).trim()] || null;
...
const FIRST = ["A VENKATA", "RAVI", "PRIYA", "SURESH", "LAKSHMI", "NARESH", "DIVYA", "KIRAN", "ANITHA", "MOHAN", "SRINIVAS", "PADMA", "RAJESH", "SWAPNA", "VENKAT", "GEETHA", "HARI", "SANDHYA", "PRASAD", "MEENA", "ARUN", "BHAVANI", "CHANDRA", "DEEPAK"];
const LAST = ["RAMANAMMA", "REDDY", "NAIDU", "RAO", "KUMAR", "PRASAD", "SHARMA", "VARMA", "CHOWDARY", "GUPTA", "BABU", "MURTHY", "SASTRY", "PATNAIK"];
...
const ROLE_WEIGHTS = [[12, 171], [7, 120], [16, 62], [8, 47], [11, 35], [14, 33], [18, 27], [3, 19], [6, 15], [9, 7], [4, 6], [15, 6], [5, 2], [19, 1]];
...
function weightedBundle() {
  const r = rnd();
  if (r < 0.74) return [...STANDARD_BUNDLE];
  if (r < 0.8) return [45, 82, 94, 113, 117, 119, 120, 129, 131];
  if (r < 0.85) return [129, 131, 133];
  if (r < 0.9) return [45, 94, 95, 113, 116, 117, 119, 120, 129, 131, 133];
  if (r < 0.93) return [103, 110];
  if (r < 0.95) return [];
  if (r < 0.97) return [82];
...
function makeMember(i) {
  ...
  const levelId = (() => { const r = rnd(); return r < 0.645 ? 5 : r < 0.87 ? 4 : 2; })();
  const location = levelId === 5 ? pick(CONSTITUENCIES) : levelId === 4 ? pick(PARLIAMENTS) : "Andhra Pradesh";
  const components = weightedBundle();
  const isAcitve = rnd() < 0.9 ? "Y" : "N";
  const hasCadre = isAcitve === "Y" ? true : rnd() > 0.11;
  const mid = String(20000 + Math.floor(rnd() * 79999));
  const mobile = hasCadre ? `9${Math.floor(100000000 + rnd() * 899999999)}` : null;
  const inserted = new Date(2023, Math.floor(rnd() * 12), 1 + Math.floor(rnd() * 27), Math.floor(rnd() * 24), Math.floor(rnd() * 60));
  return {
    activity_member_id: 1000 + i, member_name: name,
    tdp_cadre_id: hasCadre ? 500000 + i : null,
    membership_id: hasCadre ? mid : null, mobile_no: mobile,
    is_acitve: isAcitve, inserted_time: inserted.toISOString(), updated_by: 100 + Math.floor(rnd() * 20),
    role_id: roleId, role_name: role.type, role_short: role.short,
    level_id: levelId, level_name: USER_LEVELS.find((l) => l.id === levelId).name,
    location_value: location, component_ids: components,
    group_id: rnd() < 0.45 ? 1 + Math.floor(rnd() * 6) : null, // ~45% in a group
  };
}
const INITIAL_MEMBERS = Array.from({ length: 120 }, (_, i) => makeMember(i));
      ...
      <aside style={{ width: 232, flex: "none", borderRight: `1px solid ${t.divider}`, background: t.bg, position: "sticky", top: 0, height: "100vh", display: "flex", flexDirection: "column", padding: "18px 0" }}>
        ...
        <div style={{ marginTop: "auto", padding: "0 16px" }}>
          <div style={{ padding: "12px", border: `1px solid ${t.divider}`, borderRadius: 4, fontSize: 11, color: t.subtle, lineHeight: 1.5 }}>
            ...
            Mock data. Identity chain <code style={{ color: t.accent700 }}>activity_member → tdp_cadre</code>.
          </div>
        </div>
      </aside>
...
function Overview({ t, dark, stats }) {
  const kpis = [
    { label: "Total users", value: stats.total, icon: <Users size={16} />, note: "activity_member logins" },
    { label: "Active", value: stats.active, icon: <UserCheck size={16} />, note: "is_acitve = 'Y'", color: t.ok },
    { label: "Inactive", value: stats.inactive, icon: <UserX size={16} />, note: "is_acitve = 'N'", color: t.subtle },
    { label: "No components", value: stats.withoutComponents, icon: <PackageOpen size={16} />, note: "blank dashboard", color: stats.withoutComponents ? t.warn : t.ok },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 14 }}>
        {kpis.map((k) => (
          <Card key={k.label} t={t} style={{ padding: 16 }}>
            ...
            <div style={{ fontSize: 11, color: t.subtle, marginTop: 5, fontFamily: "ui-monospace,monospace" }}>{k.note}</div>
          </Card>
        ))}
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card t={t} style={{ overflow: "hidden" }}>
                ...
                <tr key={u.activity_member_id} className="ac-row" style={{ borderTop: `1px solid ${t.divider}` }}>
                  ...
                  <td onClick={() => onOpen(u.activity_member_id)} style={{ cursor: "pointer", padding: "10px 8px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      ...
                      <span>{u.member_name}</span>
                    </div>
                  </td>
                  <td style={{ padding: "10px 8px", color: u.mobile_no ? t.text : t.warn }}>{u.mobile_no || "missing"}</td>
                  <td style={{ padding: "10px 8px" }}><RoleBadge t={t} dark={dark} label={u.role_name} /></td>
                  <td style={{ padding: "10px 8px", fontSize: 12, color: t.subtle }}>{u.level_name} · {u.location_value}</td>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", borderTop: `1px solid ${t.divider}`, fontSize: 12, color: t.subtle }}>
          ...
          <span style={{ fontFamily: "ui-monospace,monospace" }}>source: activity_member</span>
        </div>
      </Card>
    </div>
  );
        ...
        <Card t={t} style={{ padding: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <h2 style={{ margin: 0, fontFamily: HEAD, fontSize: 17, lineHeight: 1.2 }}>{draft.member_name}</h2>
              <div style={{ fontSize: 12, color: t.accent, marginTop: 2 }}>{draft.role_name}</div>
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Card t={t} style={{ padding: 18 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
              {effective.length === 0 && <span style={{ fontSize: 13, color: t.warn }}>No components — this login opens to a blank dashboard.</span>}
          ...
          <Card t={t} style={{ padding: 18 }}>
            ...
            <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, fontSize: 13 }}>
              ...
              <KV t={t} k="Last updated by" v={`user #${u.updated_by}`} mono />
              <KV t={t} k="Role source" v="activity_member_access_type" mono />
              <KV t={t} k="Scope source" v="activity_member_access_level" mono />
            </div>
          </Card>
        </div>
      </div>
  ...
  return (
    <Card t={t} style={{ padding: 0, overflow: "hidden" }}>
      <div style={{ padding: "14px 18px", borderBottom: `1px solid ${t.divider}`, fontSize: 12.5, color: t.subtle, lineHeight: 1.5 }}>
        The only audit trail in this schema is <code style={{ color: t.accent700 }}>inserted_time</code> and{" "}
        <code style={{ color: t.accent700 }}>updated_by</code> on each login. There is no event-log table, so this lists login records by creation time rather than inventing a history.
      </div>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
        <tbody>
          {events.map((u) => (
            <tr key={u.activity_member_id} style={{ borderTop: `1px solid ${t.divider}` }}>
              ...
              <td style={{ padding: "11px 8px" }}>
                Login <strong>{u.member_name}</strong> ({u.membership_id || "no MID"}) created as <RoleBadge t={t} dark={dark} label={u.role_name} />
              </td>
...
function CreateModal({ t, dark, groups, onClose, onCreate, onOtp }) {
  ...
  function doLookup() {
    ...
    const found = lookupCadre(mid);
    if (found) {
      setCadre(found); setNotFound(false);
      setName(`${found.first_name} ${found.last_name}`);
      setMobile(found.mobile_no || "");
      if (found.constituency && CONSTITUENCIES.includes(found.constituency)) { setLevelId(5); setLocation(found.constituency); }
    } else {
          ...
          {step === 1 && (
            <>
              ...
              <div style={{ fontSize: 12, color: t.subtle, lineHeight: 1.5 }}>
                The ID is looked up in <code style={{ color: t.accent700 }}>tdp_cadre</code> (indexed — a point lookup, never a scan). Try <strong>20481</strong> or <strong>30017</strong> for a hit; anything else misses.
              </div>
            ...
            <>
              {cadre ? (
                <Card t={t} style={{ padding: 14, borderLeft: `3px solid ${t.ok}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: t.ok, marginBottom: 10 }}><UserCheck size={15} /> Matched in tdp_cadre — cadre #{cadre.tdp_cadre_id}</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, fontSize: 13 }}>
                    ...
                    <KV t={t} k="Constituency" v={cadre.constituency} />
